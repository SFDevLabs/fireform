...
(function($) {
    $.fireform = function(element, options) {
        ...
        $('<script>').attr('src','https://cdn.firebase.com/js/client/1.0.3/firebase.js').attr('type','text/javascrip').appendTo('body')


        that=this;
        this.formcheck=function(){
            if (!Firebase)
                setTimeout(function(){that.formcheck()},100)
            else
                this.formBind()
        }
        this.formBind=function(){
            var fb = new Firebase(options),
                target = $(element);       
            target.children(':submit').on('click', function() {
                event.preventDefault()
                var obj = {};
                //Loop through and find the inputs
